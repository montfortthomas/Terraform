...
resource "aws_dynamodb_table_item" "item_entry" {
  depends_on = [aws_dynamodb_table.menu]
  table_name = aws_dynamodb_table.menu.name
  hash_key   = aws_dynamodb_table.menu.hash_key
  count = length(local.content.Items)
  item  = <<ITEM
            ${jsonencode(local.content.Items[count.index])}
          ITEM    
}
