...
data "aws_iam_role" "api-iamrole" {
  name = "vwt-role-lex-cloudwatch"  # Require for all environments
}
...
resource "aws_api_gateway_method_response" "getTranslation" {
  ...
  status_code = "200"
  depends_on = [
    aws_api_gateway_method.getTranslation
  ]
}
...
resource "aws_api_gateway_integration_response" "getTranslation" {
  ...
  status_code = "200"
  depends_on = [
    aws_api_gateway_integration.getTranslation
  ]
}
...
resource "aws_api_gateway_account" "act" {
  cloudwatch_role_arn = data.aws_iam_role.api-iamrole.arn
}
